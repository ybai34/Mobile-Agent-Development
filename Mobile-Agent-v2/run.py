...
from MobileAgent.controller import get_screenshot, tap, slide, type, back, home, insert
...
while True:
    ...
    if "Open app" in action:
        ...
        matches = re.findall(insert_pattern, action)

        # 遍历匹配项，打印或处理数据
        for speaker, content in matches:
            print(f"获取到发言人: {speaker}, 发言内容: {content}")
            insert(speaker, content)
